...
1. **Read the patient's chip** and save the read untouched. Read it twice and compare the two reads against each other — if they differ, the read is unreliable (bad contact, weak power, a board still partly powered) and nothing after this step means anything.
2. **Open the patient and the donor** side by side ([[topic:first-comparison|comparison]]).
3. **Check the sizes** in the pane headers. Different sizes mean different chips or a wrong read — sort that out first.
...
9. **Check before flashing**: no red bytes left, the file's size unchanged, checksums right, and a final comparison against the original read where every difference is one you intended ([[topic:bench-safety|Bench rules]]).
