...
- **Edit ▸ Insert Mode** (⌥⌘I) — a typing mode where keys insert and delete instead of overwriting. It asks once per file rather than per keystroke, says so in the status bar, and changes the shape of the caret.
...
Every edit is an undo step (⌘Z), including the big ones: a join, a fill, a write made by a [[topic:tools-overview|firmware panel]], a part put back into its parent. Undo is per document.
