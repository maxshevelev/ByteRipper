...
4. Jump between the differences instead of scrolling: **⌥⌘→** goes to the next one, **⌥⌘←** to the previous one. The status bar keeps the count.
...
6. Turn on a firmware panel — **Tools ▸ UEFI Structure** — and the offsets stop being numbers: the panel names the region or the volume each address falls in.
