...
Each master carries a read mask and a write mask, one bit per [[term:region|region]]. This is why the same operation goes through from a programmer and is refused from a utility on the running board: a programmer is nobody's master, and drives the chip itself.
