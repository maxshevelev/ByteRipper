...
# Swift sources the anchors and the `L("…")` keys are read from. The package
# `.build` directories are checkouts of each other and would count everything
# several times.
SOURCE_ROOTS = ["ByteRipperApp", "Modules", "Packages"]
SKIP_PARTS = {".build", ".swiftpm", "Tests", "DerivedData"}
...
# `L("…")` and `L("…", args)`. Only a plain literal counts: a key built at run
# time is a key no translator can find, which is itself worth knowing, so those
# are reported separately rather than silently missed.
L_CALL = re.compile(r'\bL\(\s*"((?:[^"\\]|\\.)*)"(?P<context>\s*,\s*context:\s*"(?:[^"\\]|\\.)*")?', re.S)
L_DYNAMIC = re.compile(r'\bL\(\s*(?!")', re.S)
...
def unescape(text):
    """A Swift string literal's body, as the string it stands for."""
...
def collect_string_keys(root):
    ...
    for path in swift_sources(root):
        ...
        for literal, _ in L_CALL.findall("\n".join(code)):
            ...
            # which is why it is worth a check.
            if "\\(" in literal:
        ...
        for number, line in enumerate(code, 1):
            ...
            if not L_DYNAMIC.search(line) or "func L(" in line:
                continue
...
def check_strings(root):
    ...
    base = root / STRINGS
    if base.is_dir():
        for folder in sorted(base.glob("*.lproj")):
            language = folder.name[: -len(".lproj")]
            path = folder / "Localizable.strings"
            if not path.is_file():
                report[language] = (sorted(wanted), [], ["no Localizable.strings"])
                continue
