...
| 6a | Chipset | variant starts `CS/PMC/GSC` & not `PMCDG` & `pch_init_final` non-empty | `pch_init_final[-1][0]` — combined display cell `"<chipset> <letters,comma-joined>"` (e.g. "CNP/CMP-H B,A") from `pch_init_anl` 9097–9131 | [E] `mfsVolume?.pchInit?.chipsets` (row 81) — engine keeps `[("CNP/CMP-H","BA")]`; the row is every chipset as chipset + `" "` + steppings joined by `,`, the chipsets themselves joined by newline — upstream's `[-1][0]` is the total cell its aggregation appends, which is exactly that join (9126–9130) |
