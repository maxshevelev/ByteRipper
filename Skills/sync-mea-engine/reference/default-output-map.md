...
(e.g. the Chipset row ← the Intel Configuration's `mphytbl`).
...
| 5 | SKU | **hidden** for (CSTXE & `'Unknown' in sku`) · (SPS,`'NaN'`) · variant starts `PMCAPL/PMCBXT/PMCGLK/PCHC/PMCDG/OROM` (13694–13698) | `sku` — per-family SKU block (legacy literal tables 12680–13231; CSME≥12 via `get_csme12_sku` 10287 / FW-SKU decode 13120–13127) | [E] `sku` — `SKU.csme` for CSME 11 and up (the 0x0C/0x0F label, then the platform letter: CSME 11 from the 0x0C SKU Platform field with the database row as fallback, CSME 12+ the other way round), `PreCSEME` for the ME family. Oracle-verified: "Corporate LP" on CSME 11.8, "Consumer H" on 12.0 and 16.1, "Slim" on ME 7. Not ported: the pre-11.0.0.1205 kernel-module Huffman scan, whose firmware falls back to its database row |
| 6a | Chipset | variant starts `CS/PMC/GSC` & not `PMCDG` & `pch_init_final` non-empty | `pch_init_final[-1][0]` — combined display cell `"<chipset> <letters,comma-joined>"` (e.g. "CNP/CMP-H B,A") from `pch_init_anl` 9097–9131 | [E] `chipsetInit?.chipsets` (row 81; `mfsVolume.pchInit` is the volume's own copy, which the FTPR `intl.cfg` may have replaced) — engine keeps `[("CNP/CMP-H","BA")]`; the row is every chipset as chipset + `" "` + steppings joined by `,`, the chipsets themselves joined by newline — upstream's `[-1][0]` is the total cell its aggregation appends, which is exactly that join (9126–9130) |
...
| 11 | Production Ready | `pvbit is not None` | `['No','Yes'][pvbit]` — FTPR manifest header production-ready flag (`mn2_flags_pvbit`, 12659; ME/TXE use the `$DAT…IFRP` probe 12652–12655) | [E] `ManifestSummary.productionReady` — Flags bit 0 on an R1/R2 manifest, and the `$DAT` + 20 bytes + `IFRP` probe on a pre-CSE ME 8–10 / TXE one (`PreCSEME.productionReady`). An ME before major 8 carries no such bit, and the row is absent there as it is in the console |
...
| 13 | Patsburg Support | (ME, 7) | `['No','Yes'][is_patsburg]` | [E] `patsburgSupport` — the `$SKU` attributes' Patsburg bit (`PreCSEME`, byte 4 bit 7), filled for ME 7–8 and read by the row for ME 7. Oracle-verified on `ME 7.bin` ("No", platform CPT) |
...
| 17 | File System State | variant in `CSME/CSTXE/CSSPS/GSC` | `mfs_state` — `'Initialized'/'Configured'/'Unconfigured'/'Error'` from the MFS scan (`mfs_parsed_idx` 7489–7493; init 11075; raised by EFS/config presence 13050–13051 etc.) | [E] `mfsState` — the reserved-index rule (7489–7490), the EFS raise (13050, `efs_init` — the volume's decoded file list, whose last file having bytes is upstream's own `bool(file_data_all)`) and the configuration raise (13051: a non-empty `fitc.cfg` module or FITC/CDMD/MFSB partition), reached through upstream's own `elif` so the EFS raise wins. Verified on all six dumps: `CSME 11.bin`/`CSME 12.BIN`/`CSME 15.bin`/`CSME16.1.BIN` Initialized, `CSME 16.bin` Configured — every one as the console |
...
| 21 | Downgrade Blacklist 7.0 / 7.1 | (ME, 7) | `me7_blist_1` / `me7_blist_2` | [E] `downgradeBlacklist` — the two minor/hotfix/build triples at 0x6DF / 0x6EB past the manifest tag (upstream's `start_man_match` = manifest base + 0x1B); a zero build word is the "Empty" the row prints. Oracle-verified on `ME 7.bin`: `<= 7.0.0.1172` and Empty |
| 22 | Chipset Support | `platform != 'NaN'` | `platform` — per-family PCH/Southbridge text (legacy literal tables 12748–12913, e.g. `ICH8`/`IBX`) | [E] `platform` — pre-CSE families from `PreCSEME`, IUP images from their descriptor, and the CSE ones from `CSEPlatformNames`: the CSME table (11–16) gated on there being no chipset-init table, plus CSTXE's three. The gate is now exact — both streams that can hold such a table are read by the time it is asked, so it is simply whether the aggregate is empty. Verified against the console on all six dumps: named only on `CSME 16.bin` ("ADP/RPP", whose `intl.cfg` lists a zero-length `mphytbl` upstream skips), absent on the rest. Not ported: the (CS)SPS `CSE_Ext_50` SKU-platform names and the GSC ones — those stay nil, and the row absent |
...
With every row in, the panel's summary was diffed against the original
script's console output over the dumps in `~/Desktop/ME`:

- **ME 7.bin** — identical, all 11 rows: the pre-CSE set, Patsburg Support and
  both Downgrade Blacklist lines among them.
- **CSME 11.bin** — identical, all 16 rows: "Corporate LP", the Power Down
  Mitigation and Workstation Support pair, and *no* ARB Security Version
  Number — that row belongs to CSME 12 and later, and the console omits it
  here too.
- **CSME 12.BIN** — identical, all 30 lines: the 16 main rows and the whole
  Power Management Controller block.
- **CSME 16.bin** — identical, all 74 lines: the 17 main rows, four
  independent blocks (PMC, PCHC and two PHY) and the trailing message.

The ME-7 diff is what caught the last three gates: rows 8/9/10 were printed
for every identified image, where the console gives the TCB and VCN pair only
to an ME from major 8 (and to every TXE/CSE/GSC/IUP firmware) and the ARB
number only from CSME 12 — and row 11 asks whether the firmware carries a
production bit at all.
...
| **ADD — byte-core fact** | — | Nothing open. File System State's EFS leg (17) closed on 2026-09-24 with the rest of the `FileTable.dat` work: `efs_init` reads the EFS file list the EFST table already gives the volume. Rows 9, 10, 11, 17, 18 and 20 are all surfaced (`arbSvn`, `vcn`, `ManifestSummary.productionReady`, `mfsState`, `firmwareSizeBytes`, `version.me*`). |
| **ADD — fixture-only later** | — | Nothing: rows 13 and 21 are oracle-verified on `ME 7.bin` and the CSME-11 rows on the CSME 11.8 dump. A Patsburg "Yes" would still need an X79/C600 image, and the pre-CSE production bit an ME 8–10 or TXE one — both only to see the other branch of a rule already read from bytes. |
