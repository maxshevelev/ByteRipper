...
Pairs with the entry above: memoizing the databases removes every wait after
the first one in a run, and this entry explains the first one — which is the one
that cannot be removed, only named.
